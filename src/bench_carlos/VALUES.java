...
public class VALUES implements JoinOperator
{
    ...
    public BenchmarkResult executeHTTP(SplitQuery sq, boolean debug)
    {
        ...
        try
        {
            ...
            TreeSet<BindingHashMap> resultsP1 = Tasks
                    .service(sq.getP1Endpoint(), p1);
            ...
            for (String varString : QueryParser
                    .findJoinVars(p1.getProjectVars(), p2.getProjectVars()))
            {
                match_vars.add(Var.alloc(varString));
            }

            TreeSet<BindingHashMap> resultsP2 = Tasks.serviceWithValues(
                    sq.getP2Endpoint(), p2, resultsP1, match_vars,
                    sq.getBatch());
