...
public abstract class CLIObject
{
    ...
    /**
     * add all Option(Groups) to this object Note: The help flag is set
     * automatically ("?")
     * 
     * @param opts
     */
    ...
    protected CommandLine verifyArgs(String[] args)
    {
        ...
        if (args.length == 0 || cmd.hasOption(PARAMETERS.PARAM_HELP)
                || cmd.hasOption(PARAMETERS.PARAM_HELP1))
        {
            HelpFormatter formatter = new HelpFormatter();
            formatter.printHelp(this.getClass().getSimpleName(), _opts, true);
            System.exit(-1);
